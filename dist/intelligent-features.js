 ...
 */
import { TaskStatus, AgentRole, Phase, createTask, createNote, } from './models.js';
/**
 * Analyze the project's task dependency graph and find critical path
 */
export function analyzeCriticalPath(context) {
    ...
    // Estimate completion
    const remaining = context.tasks.filter((t) => t.status !== TaskStatus.COMPLETED && t.status !== TaskStatus.CANCELLED);
    return {
 ...
 */
export function getSmartPriorityQueue(context, limit = 5) {
    ...
    for (const task of analysis.readyTasks) {
        ...
        const unblockCount = context.tasks.filter((t) => t.dependencies.includes(task.id) && t.status === TaskStatus.PENDING).length;
 ...
 */
export function decompressContext(compressed) {
    return {
        id: compressed.id,
        name: compressed.n,
...
export function calculateHealthScore(context) {
    ...
    const overall = Math.round(velocityScore * weights.velocity +
 ...
 */
export function executeBatchOperations(context, operations) {
    ...
    for (let i = 0; i < operations.length; i++) {
        ...
        try {
            switch (op.type) {
                case 'create_task': {
                    const task = createTask(op.payload.title, op.payload.description, op.payload.options);
                ...
                case 'add_note': {
                    const note = createNote(op.payload.agent, op.payload.content, op.payload.category);
                ...
                case 'update_status': {
                    ...
                    currentContext.tasks[statusIndex] = {
                        ...
                        updatedAt: now,
                        completedAt: op.payload.status === TaskStatus.COMPLETED
                            ? now
 ...
 */
export function generateTaskSuggestions(context) {
    const suggestions = [];
    const analysis = analyzeCriticalPath(context);
 ...
 */
export function createStreamEvent(type, projectId, data) {
    return {
...
//# sourceMappingURL=intelligent-features.js.map