...
3. Suggest a priority order for execution`,
        variables: ['project_name', 'phase', 'task_count', 'completed_count', 'blocker_count', 'description'],
...
3. Recommend fixes`,
        variables: ['project_name', 'current_task', 'task_description', 'problem_description', 'attempted_solutions'],
...
{{next_priority}}`,
        variables: ['project_name', 'description', 'phase', 'completed_tasks', 'pending_tasks', 'notes', 'next_priority'],
...
### Notes:
{{notes}}`,
        variables: ['date', 'project_name', 'yesterday_completed', 'today_planned', 'blockers', 'notes'],
...
export async function getTimeStats(projectId) {
    ...
    const averageSessionLength = durations.length > 0
...
${pendingTasks.map((t) => `### ${t.status === 'in_progress' ? '🔄' : t.status === 'blocked' ? '🚫' : '📋'} ${t.title}
- Priority: ${t.priority}
- Status: ${t.status}
...
${context.notes.map((n) => `### ${n.category.toUpperCase()} (${n.agent})
${n.content}
`).join('\n') || 'No notes yet'}
...
${context.tasks.map((t) => `### ${getStatusEmoji(t.status)} ${t.title}
...
${context.notes.map((n) => `### ${getCategoryEmoji(n.category)} ${n.category.toUpperCase()}
