...
async function handleContext(req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  if (req.method === 'PUT') {
    ...
    if (body.phase) {
      context = setPhase(context, body.phase as Phase);
    }
    if (body.name) {
...
async function handleTasks(req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  if (req.method === 'PUT' && pathParts.length === 3) {
    ...
    if (body.status) {
      context = updateTaskStatus(context, taskId, body.status as TaskStatus);
    }
    if (body.note) {
...
async function handleAnalytics(req: IncomingMessage, res: ServerResponse): Promise<void> {
  const storage = await getStorage();
  const url = new URL(req.url ?? '/', `http://localhost:${PORT}`);
...
async function handleClone(req: IncomingMessage, res: ServerResponse): Promise<void> {
  const storage = await getStorage();
...
async function handleCriticalPath(req: IncomingMessage, res: ServerResponse): Promise<void> {
  const storage = await getStorage();
  const url = new URL(req.url ?? '/', `http://localhost:${PORT}`);
  const projectId = url.searchParams.get('project_id');
...
async function handleSmartQueue(req: IncomingMessage, res: ServerResponse): Promise<void> {
  const storage = await getStorage();
  const url = new URL(req.url ?? '/', `http://localhost:${PORT}`);
  const projectId = url.searchParams.get('project_id');
...
async function handleCompress(req: IncomingMessage, res: ServerResponse): Promise<void> {
  const storage = await getStorage();
  const url = new URL(req.url ?? '/', `http://localhost:${PORT}`);
  const projectId = url.searchParams.get('project_id');
...
async function handleHealthScore(req: IncomingMessage, res: ServerResponse): Promise<void> {
  const storage = await getStorage();
  const url = new URL(req.url ?? '/', `http://localhost:${PORT}`);
  const projectId = url.searchParams.get('project_id');
...
async function handleBatch(req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  if (req.method === 'POST') {
    ...
    const result = executeBatchOperations(context, operations as BatchOperation[]);
...
async function handleSuggestions(req: IncomingMessage, res: ServerResponse): Promise<void> {
  const storage = await getStorage();
  const url = new URL(req.url ?? '/', `http://localhost:${PORT}`);
  const projectId = url.searchParams.get('project_id');
...
async function handleWebhooks(req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  if (req.method === 'POST' && pathParts.length === 2) {
    ...
    const webhook = createWebhook(webhookUrl, events as EventType[], { secret });
...
async function handleTemplates(req: IncomingMessage, res: ServerResponse): Promise<void> {
  const storage = await getStorage();
  const advanced = await getAdvancedStorage();
  const url = new URL(req.url ?? '/', `http://localhost:${PORT}`);
  const pathParts = url.pathname.split('/').filter(Boolean);
...
async function handleSnapshots(req: IncomingMessage, res: ServerResponse): Promise<void> {
  const storage = await getStorage();
  const advanced = await getAdvancedStorage();
  const url = new URL(req.url ?? '/', `http://localhost:${PORT}`);
  const pathParts = url.pathname.split('/').filter(Boolean);
...
async function handlePersonalTodos(req: IncomingMessage, res: ServerResponse): Promise<void> {
  const url = new URL(req.url ?? '/', `http://localhost:${PORT}`);
  ...
  if (req.method === 'POST') {
    ...
    const todo = await addPersonalTodo(body.content as string, {
...
async function handlePersonalTodoComplete(
  req: IncomingMessage,
  res: ServerResponse
): Promise<void> {
  const url = new URL(req.url ?? '/', `http://localhost:${PORT}`);
  ...
  if (req.method === 'POST') {
    ...
    const did = await completeTodo(todoId, {
      reflection: body.reflection as string | undefined,
...
async function handleDids(req: IncomingMessage, res: ServerResponse): Promise<void> {
  const url = new URL(req.url ?? '/', `http://localhost:${PORT}`);
...
async function handleGoals(req: IncomingMessage, res: ServerResponse): Promise<void> {
  if (req.method === 'GET') {
  ...
  if (req.method === 'PUT' || req.method === 'POST') {
    const body = await parseBody(req);
    if (body.daily) await setDailyGoals(body.daily as string[]);
    if (body.weekly) await setWeeklyGoals(body.weekly as string[]);
    const goals = await getGoals();
    json(res, goals);
    return;
  }
...
async function handleMemory(req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  if (req.method === 'POST') {
    ...
    const memory = await remember(body.key as string, body.value as string, {
...
async function handleTimeTracking(req: IncomingMessage, res: ServerResponse): Promise<void> {
  const url = new URL(req.url ?? '/', `http://localhost:${PORT}`);
  ...
  if (url.pathname.endsWith('/stop') && req.method === 'POST') {
    const body = await parseBody(req);
    const entry = await stopTimeTracking(body.notes as string | undefined);
    if (!entry) {
...
async function handlePromptTemplates(req: IncomingMessage, res: ServerResponse): Promise<void> {
  const storage = await getStorage();
  const url = new URL(req.url ?? '/', `http://localhost:${PORT}`);
  ...
  if (url.pathname.endsWith('/generate') && req.method === 'POST') {
    ...
    const prompt = generatePromptFromContext(context, body.template_id as string);
    if (!prompt) {
      error(res, 'Template not found', 404);
      return;
    }
...
async function handleExportMd(req: IncomingMessage, res: ServerResponse): Promise<void> {
  const storage = await getStorage();
  const url = new URL(req.url ?? '/', `http://localhost:${PORT}`);
  ...
  if (req.method === 'GET' || req.method === 'POST') {
    const projectId = url.searchParams.get('project_id');
    const format =
      (url.searchParams.get('format') as 'minimal' | 'standard' | 'detailed') ?? 'standard';
    ...
    if (req.method === 'POST') {
      const body = await parseBody(req);
      if (body.save_path) {
        await saveClaudeMd(context, body.save_path as string, format);
...
async function handleProductivityStats(req: IncomingMessage, res: ServerResponse): Promise<void> {
  const url = new URL(req.url ?? '/', `http://localhost:${PORT}`);
...
async function handleRAGDocuments(req: IncomingMessage, res: ServerResponse): Promise<void> {
  const url = new URL(req.url ?? '/', `http://localhost:${PORT}`);
  const parts = url.pathname.split('/');
...
async function handleRAGIndex(req: IncomingMessage, res: ServerResponse): Promise<void> {
  const url = new URL(req.url ?? '/', `http://localhost:${PORT}`);
  ...
  if (url.pathname.endsWith('/document') && req.method === 'POST') {
    ...
    const doc = await indexDocument(body.title as string, body.content as string, {
      projectId: body.project_id as string | undefined,
...
async function handleRAGSearch(req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  if (req.method === 'GET' || req.method === 'POST') {
    ...
    let searchType: 'vector' | 'keyword' | 'hybrid' | undefined;
    ...
    if (req.method === 'GET') {
      ...
      searchType = (url.searchParams.get('search_type') as any) ?? undefined;
      ...
      projectId = body.project_id as string | undefined;
      searchType = body.search_type as any;
      topK = body.top_k as number | undefined;
...
async function handleRAGContext(req: IncomingMessage, res: ServerResponse): Promise<void> {
  const url = new URL(req.url ?? '/', `http://localhost:${PORT}`);
  ...
  if (req.method === 'GET' || req.method === 'POST') {
    ...
    let includeMetadata: boolean | undefined;
    ...
    if (req.method === 'GET') {
      ...
      query = body.query as string;
      projectId = body.project_id as string | undefined;
      maxContextLength = body.max_context_length as number | undefined;
      includeMetadata = body.include_metadata as boolean | undefined;
    }
...
async function handleRAGStats(req: IncomingMessage, res: ServerResponse): Promise<void> {
  if (req.method === 'GET') {
...
async function handleRAGConfig(req: IncomingMessage, res: ServerResponse): Promise<void> {
  if (req.method === 'GET') {
  ...
  if (req.method === 'PUT' || req.method === 'POST') {
    ...
    if (body.embedding) {
      ...
      updates.embedding = {
        provider: e.provider as EmbeddingProvider | undefined,
    ...
    if (body.chunking) {
      const c = body.chunking as Record<string, unknown>;
      updates.chunking = {
        strategy: c.strategy as ChunkingStrategy | undefined,
        chunkSize: c.chunk_size as number | undefined,
        chunkOverlap: c.chunk_overlap as number | undefined,
      };
    }
    ...
    if (body.search) {
      ...
      updates.search = {
        topK: s.top_k as number | undefined,
        minScore: s.min_score as number | undefined,
        hybridVectorWeight: s.hybrid_vector_weight as number | undefined,
      };
    }
