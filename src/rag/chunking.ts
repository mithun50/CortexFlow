 ...
 */
function chunkByParagraph(text: string, config: ChunkingConfig): ChunkResult[] {
 ...
 */
function chunkBySentence(text: string, config: ChunkingConfig): ChunkResult[] {
 ...
 */
function chunkByFixed(text: string, config: ChunkingConfig): ChunkResult[] {
  const chunks: ChunkResult[] = [];
  let start = 0;
  let chunkIndex = 0;
 ...
 */
function chunkBySemantic(text: string, config: ChunkingConfig): ChunkResult[] {
  // Detect markdown headers
  const headerRegex = /^#{1,6}\s+.+$/gm;
  const headers: { match: string; index: number }[] = [];
  let match;
 ...
 */
export function chunkDocument(text: string, config: ChunkingConfig): ChunkResult[] {
 ...
 */
export function getRecommendedChunkSize(model: string): number {
 ...
 */
export function mergeSmallChunks(chunks: ChunkResult[], minSize: number): ChunkResult[] {
 ...
 */
export function splitOversizedChunk(chunk: ChunkResult, maxSize: number): ChunkResult[] {
  if (chunk.content.length <= maxSize) {
    return [chunk];
  }

  const pieces: ChunkResult[] = [];
  let start = 0;
  let pieceIndex = 0;
